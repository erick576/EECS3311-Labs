note
	description: "Summary description for {PERSONAL_TESTS}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
...
feature -- model
	...
	dbt_model7: BOOLEAN
		...
		do
			...
			if
				attached {LINEAR_DB[INTEGER, STRING]} db_int_str as a_db
			then
				Result :=
					across
						a_db.model is i_pair
	...
	dbt_model5: BOOLEAN
		...
		do
			...
			if
				attached {TREE_DB[INTEGER, STRING]} db_int_str as a_db
			then
				Result :=
					across
						a_db.model is i_pair
			...
			then
				...
				Result :=
					...
					all
						...
						a_range.count = 1
						and then
						attached a_db.values[i_key] as mapped
						and then
						a_range.has (mapped)
					end
			end
	...
	dbt_model1: BOOLEAN
		local
			...
			l_bst_ss: BALANCED_BST[STRING, STRING]
			l_list_ss: LIST[TREE_NODE[STRING, STRING]]

			l_bst_is: BALANCED_BST[INTEGER, STRING]
			l_list_is: LIST[TREE_NODE[INTEGER, STRING]]
		do
			...
			then
				...
				Result :=
					...
					all
						...
						implies
						(
							attached a_key as key
							and then
							attached a_db.model.domain_restricted_by(key).range as a_range
							and then
							a_range.count = 1
							and then
							attached a_value as value
							and then
							a_range.has(value)
						)
					end
	...
	add_iterator_tests
 	...
 	dbt_iterator2: BOOLEAN
		local
	 	...
	 	do
	 		comment ("dbt_iterator2: TREE_DB, int_int, random operations")
			reset_as_tree_db

			create {ARRAYED_LIST[TUPLE[INTEGER, INTEGER]]} l_list.make (db_int_int.count)

			db_int_int.insert (10, 10)
			db_int_int.insert (9, 9)
			db_int_int.insert (8, 8)
			db_int_int.insert (7, 7)
			db_int_int.insert (6, 6)
			db_int_int.insert (5, 5)
			...
			create indexes.make_empty

			indexes.force (10, indexes.count + 1)
			indexes.force (9, indexes.count + 1)
			indexes.force (8, indexes.count + 1)
			indexes.force (7, indexes.count + 1)
			indexes.force (6, indexes.count + 1)
			indexes.force (5, indexes.count + 1)
			indexes.force (4, indexes.count + 1)
			indexes.force (2, indexes.count + 1)
			indexes.force (1, indexes.count + 1)
 	...
 	dbt_iterator3: BOOLEAN
		local
			...
			indexes : ARRAY[INTEGER]
	 	do
	 		comment ("dbt_iterator3: LINEAR_DB, int_int, random operations")
			reset_as_linear_db
